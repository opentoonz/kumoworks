...
RenderPopup::RenderPopup(QWidget* parent) : QDialog(parent, Qt::CustomizeWindowHint | Qt::WindowTitleHint){
  std::cout << "renderpopup ctor" << std::endl;

  m_targetCombo = new QComboBox(this);
  m_regionCombo = new QComboBox(this);
  m_bgCombo     = new QComboBox(this);
  m_renderQualityCombo = new QComboBox(this);
  m_renderHiddenLayers = new QCheckBox( tr("Render hidden layers as well"), this);
  ...
  m_targetCombo->addItem(tr("Combine all clouds to one image"), CombineAllCloud);
  ...
  {
    mainLay->addWidget(new QLabel(tr("Target:"), this), 0, 0, Qt::AlignRight | Qt::AlignVCenter);
    mainLay->addWidget(m_targetCombo, 0, 1);

    mainLay->addWidget(m_renderHiddenLayers, 1, 1, Qt::AlignLeft | Qt::AlignVCenter);

    mainLay->addWidget(new QLabel(tr("Region:"), this), 2, 0, Qt::AlignRight | Qt::AlignVCenter);
    mainLay->addWidget(m_regionCombo, 2, 1);

    mainLay->addWidget(new QLabel(tr("Background:"), this), 3, 0, Qt::AlignRight | Qt::AlignVCenter);
    mainLay->addWidget(m_bgCombo, 3, 1);

    mainLay->addWidget(new QLabel(tr("Rendering quality:"), this), 4, 0, Qt::AlignRight | Qt::AlignVCenter);
    mainLay->addWidget(m_renderQualityCombo, 4, 1);

    mainLay->addWidget(new QLabel(tr("Folder:"), this), 5, 0, 1, 2, Qt::AlignLeft | Qt::AlignVCenter);
    
    mainLay->addWidget(m_dirLabel, 6, 0, 1, 2);

    mainLay->addWidget(new QLabel(tr("File name:"), this), 7, 0, Qt::AlignRight | Qt::AlignVCenter);
  ...
  qRegisterMetaType<RenderWorkerThread::RenderResult>("RenderWorkerThread::RenderResult");

  connect(m_renderButton, SIGNAL(clicked()), this, SLOT(onRenderButtonClicked()));
  connect(m_targetCombo, SIGNAL(currentIndexChanged(int)), this, SLOT(onTargetChanged(int)));
...
void RenderPopup::showEvent(QShowEvent *event) {
  ...
  m_renderQualityCombo->setCurrentIndex(m_renderQualityCombo->findData(ro.voxelSkip));
...
void RenderPopup::onRenderButtonClicked() {
  if (m_dirLabel->getPath().isEmpty() || m_fileNameEdit->text().isEmpty()) {
    QMessageBox::warning(this, tr("Warning"), tr("Output directory or filename is empty."));
    return;
  }
  ...
  if(renderCloudCount == 0 && (RO_Background)(m_bgCombo->currentData().toInt()) != RenderSkySeparately) {
    QMessageBox::warning(this, tr("Warning"), tr("No clouds to be rendered in this scene."));
    return;
  }
  ...
  connect(m_renderWorkerThread, &RenderWorkerThread::renderFinished, this, &RenderPopup::onRenderFinished);
  connect(m_renderWorkerThread, SIGNAL(cloudRendered()), this, SLOT(advanceProgressBar()));
  
  m_renderWorkerThread->start();
}

void RenderPopup::onTargetChanged(int index) {
  // Choosing the current cloud option does render regardless of the "hidden layer" checkbox.
...
void RenderPopup::onRenderFinished(RenderWorkerThread::RenderResult result) {
  if (result == RenderWorkerThread::Finished) {
    ...
    if (result == RenderWorkerThread::Failed)
      QMessageBox::critical(this, tr("Critical"), tr("Failed to render clouds."));
    else if (result == RenderWorkerThread::Canceled)
      QMessageBox::information(this, tr("Information"), tr("Rendering canceled."));
